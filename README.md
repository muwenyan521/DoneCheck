...
阶段 5 引入的 `@donecheck/provider-openai` 提供 `createProvider()` 工厂，CLI（默认管线 / `--rules` / `--html`）与 desktop IPC 都通过它获取 LLM provider。未配置 `OPENAI_API_KEY` 且未传 `--mock` 时，`createProvider()` 抛出 `ProviderConfigError`，避免静默产出假结果。

### 环境变量

| 变量 | 必填 | 说明 |
| --- | --- | --- |
| `OPENAI_API_KEY` | 启用真实 provider 时必填 | 未设置或为空时须传 `--mock` 才能运行；否则报错退出 |
...
未设置 `OPENAI_API_KEY` 时须追加 `--mock` 以使用确定性 mock provider 完成完整管线，适合离线结构与退出码验收。

### Provider 工厂契约

`createProvider({ stderr, mock })` 的行为：

- `mock: true` → 向 `stderr` 写入提示，返回 `createDeterministicMockProvider()`。
- `mock: false`（默认）且 `OPENAI_API_KEY` 非空 → 返回真实 `OpenAIProvider`。
- `mock: false` 且 `OPENAI_API_KEY` 未设置或为空 → 向 `stderr` 写入错误信息，抛出 `ProviderConfigError`。

`OpenAIProvider` 构造时若缺少 API key 同样抛 `ProviderConfigError`。
...
未设置 `OPENAI_API_KEY` 时，须追加 `--mock` 以使用 deterministic mock provider，可用于离线验证完整结构化路径、文件输出和退出码：

```bash
nix develop -c bash -lc '
  env -u OPENAI_API_KEY -u OPENAI_BASE_URL -u OPENAI_MODEL \
    node apps/cli/dist/index.js --rules --mock \
...
`--rules` 输出完整 rules JSON 报告，适合保存、复核和二次校验。未设置 `OPENAI_API_KEY` 时追加 `--mock`：

```bash
nix develop -c node apps/cli/dist/index.js --rules --mock \
  --workspace fixtures/demo-react-app/workspace \
  --requirement-file fixtures/demo-react-app/inputs/requirements.md \
  --evidence-file fixtures/demo-react-app/inputs/claim.md \
...
`--html` 生成静态 HTML 报告，可通过 `--output` 指定文件路径。未设置 `OPENAI_API_KEY` 时追加 `--mock`：

```bash
nix develop -c node apps/cli/dist/index.js --html --mock \
  --workspace fixtures/demo-react-app/workspace \
  --requirement-file fixtures/demo-react-app/inputs/requirements.md \
  --evidence-file fixtures/demo-react-app/inputs/claim.md \
...
```bash
nix develop -c node apps/cli/dist/index.js --rules --confirm-requirements --mock \
  --workspace fixtures/demo-react-app/workspace \
  --requirement-file fixtures/demo-react-app/inputs/requirements.md \
  --evidence-file fixtures/demo-react-app/inputs/claim.md
```
